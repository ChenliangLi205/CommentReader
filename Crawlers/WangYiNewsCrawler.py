...
class News163Crawler(object):
    ''' Crawl news.163.com and return the news and comments in the first page '''
    def __init__(self):
        self.headers = {
    ...
    def GetPageData(self, url, timeout=10, decode='gbk', waiting_secs=9):
        time.sleep(waiting_secs)
        request = urllib.request.Request(url=url, data=None, headers=self.headers)
        try:
    ...
    def ExtractNewsLinksFrom(self, homepage="https://news.163.com"):
        homePageData = self.GetHomePageDataFromChrome(homepage)
    ...
    def GetArticleNewsCommentJson(self, url, commentsNumberLimit=40):
        newsId = self.GetArticleNewsId(url)
        ...
        jsonPageData = self.GetPageData(commentJsonUrl, decode='utf-8')
        return jsonPageData

    def SplitNewsIntoArticlePhoto(self, newsUrls, newsTitles):
        ''' Return a url->title dictionary'''
        articleBasedUrl2Title = dict()
        photoBasedUrl2Title = dict()
        for url, title in zip(newsUrls, newsTitles):
            newsType = self.GetNewsType(url)
            if newsType == 'photo':
    ...
    def GetJasonDicts(self, urls):
        ''' Get Jason file and turn it into dict for article based news'''
        url2dict = dict()
        for i, url in enumerate(urls):
            if url in url2dict:
                continue
            commentJson = self.GetArticleNewsCommentJson(url)
            if commentJson is None:
                continue
            commentJson = self.JsonStripParenthese(commentJson)
            jsonDict = json.loads(commentJson)
            url2dict[url] = jsonDict
        return url2dict

    def GetCommentsfromUrls(self, urls):
        '''Get comment strings from news urls'''
        url2dict = self.GetJasonDicts(urls)
        url2comments = self.GetContentfromJasonDicts(url2dict)
        return url2comments

    def run(self):
        newsUrls, newsTitles = self.ExtractNewsLinksFrom()
        articleUrl2Title, photoUrl2Title = \
            self.SplitNewsIntoArticlePhoto(newsUrls, newsTitles)
        url2comments = self.GetCommentsfromUrls(articleUrl2Title.keys())
        returns = [] # list[(url, title, comment)]
        for url in url2comments:
            if url not in articleUrl2Title:
                print("Some Thing Ain't Right")
                continue
            print("标题：%s" % articleUrl2Title[url])
            title = articleUrl2Title[url]
            for comment in url2comments[url]:
                returns.append((url, title, comment))
        return returns